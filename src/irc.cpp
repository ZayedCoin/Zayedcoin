...
// Copyright (c) 2011-2012 Zayedcoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("Zayedcoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ZayedcoinTEST0\r");
            Send(hSocket, "WHO #ZayedcoinTEST0\r");
        } else {
            // randomly join #Zayedcoin00-#Zayedcoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Zayedcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Zayedcoin%02d\r", channel_number).c_str());
        }
