...
uint256 hashGenesisBlock("0x00000c4d2b780b14852b4c6f112b9267f204578ae68de2e612e27b601e204074");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20);
static CBigNum bnStartDifficulty(~uint256(0) >> 27);
...
const string strMessageMagic = "Zayedcoin Signed Message:\n";
...
{
    // Zayedcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	// Subsidy halving
    nSubsidy >>= (nHeight / 116850);
	
	if (nHeight > 116850*64)
		return nFees;
...
{
    // Zayedcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ZYD Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x9f77587fa53dac8eaf0f7a80da10fab6cec101dfc977f433571d86e2909ccba1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd0, 0xce, 0xcf, 0x9c };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZayedcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZayedcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZayedcoinMiner(CWallet *pwallet)
{
    printf("ZayedcoinMiner not supported!\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Zayedcoin-miner");
    ...
    try { loop {
        while (1 == 1) //while (vNodes.empty())
            MilliSleep(1000);
    } }
    catch (boost::thread_interrupted)
    {
        printf("ZayedcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZayedcoinMiner, pwallet));
}
